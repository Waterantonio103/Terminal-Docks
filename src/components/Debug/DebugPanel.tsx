...
function buildCodexDebugPrompt(options: {
  ...
  return [
    ...
    '3. Inspect failures using debug_get_recent_runtime_logs, debug_get_workflow_events, debug_get_terminal_tail, debug_search_logs, debug_list_ui_screenwatch_snapshots, debug_read_ui_screenwatch_snapshot, debug_capture_app_screenshot, debug_search_code, and debug_read_file as needed. Treat screenwatch JSON as DOM/xterm heuristics only; it cannot prove the UI is visually clean. For UI validation, explicitly call debug_capture_app_screenshot, open/analyze the PNG, and report perceivable UI errors such as broken layout, overlapping panes, clipped text, blank areas, stale prompts, or incorrect CLI surfaces. debug_capture_app_screenshot captures the already-running CometAI app window by handle, not foreground desktop pixels, so overlapping windows should not affect the image; launch the app explicitly when UI evidence is required.',
