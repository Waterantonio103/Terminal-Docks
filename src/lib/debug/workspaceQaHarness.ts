...
export async function runWorkspaceQaHarness(options: WorkspaceQaOptions): Promise<void> {
  ...
  }, 8_000, 'editor file tabs');

  await waitFor(() => document.querySelectorAll('.cm-editor').length >= 1, 8_000, 'CodeMirror editor');
  await waitFor(() => document.querySelectorAll('.cm-gutters').length >= 1, 8_000, 'CodeMirror gutters');
