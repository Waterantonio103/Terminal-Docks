...
export const geminiAdapter: CliAdapter = {
  ...
  detectReady(_output: string): ReadyDetectionResult {
    return { ready: false, confidence: 'low', detail: 'Gemini readiness detection not implemented' };
  },
  ...
  normalizeOutput(output: string): RuntimeOutputEvent[] {
    return [{ kind: 'unknown', cli: 'gemini', timestamp: Date.now(), detail: output.slice(0, 200) }];
  },
