...
export const opencodeAdapter: CliAdapter = {
  ...
  detectReady(_output: string): ReadyDetectionResult {
    return { ready: false, confidence: 'low', detail: 'OpenCode readiness detection not implemented' };
  },
  ...
  normalizeOutput(output: string): RuntimeOutputEvent[] {
    return [{ kind: 'unknown', cli: 'opencode', timestamp: Date.now(), detail: output.slice(0, 200) }];
  },
