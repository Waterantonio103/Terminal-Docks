...
impl AppState {
    pub fn state<T: Any + Send + Sync + 'static>(&self) -> Arc<T> {
        let state: Arc<dyn Any + Send + Sync> =
            if TypeId::of::<T>() == TypeId::of::<pty::PtyState>() {
                self.pty_state.clone()
            } else if TypeId::of::<T>() == TypeId::of::<agent_run::AgentRunState>() {
                self.agent_run_state.clone()
            } else if TypeId::of::<T>() == TypeId::of::<mcp::McpState>() {
                self.mcp_state.clone()
            } else if TypeId::of::<T>() == TypeId::of::<swarm::WatcherState>() {
                self.watcher_state.clone()
            } else if TypeId::of::<T>() == TypeId::of::<workflow_engine::WorkflowState>() {
                self.workflow_state.clone()
            } else if TypeId::of::<T>() == TypeId::of::<db::DbState>() {
                self.db_state.clone()
            } else {
                panic!("Unsupported AppState type requested");
            };

        state
            .downcast::<T>()
            .expect("AppState type check and downcast disagree")
    }
}
        ...
        match req.cmd.as_str() {
            "export_workflow_log" => {
                ...
                struct Args {
                    ...
                    pub results: Vec<ResultExport>,
                }
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(workflow_log::export_workflow_log(
                    app.clone(),
            ...
            "append_mission_patch" => {
                ...
                struct Args {
                    pub mission_id: String,
                    pub run_version: u32,
                    pub patch: MissionGraphPatch,
                }
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(workflow_engine::append_mission_patch(
                    app.clone(),
                    args.mission_id,
                    args.run_version,
                    args.patch,
            ...
            "get_mission_activations" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(workflow_engine::get_mission_activations(
                        app.clone(),
                        args.mission_id,
                    )?)
                    .map_err(|e| e.to_string())?,
            ...
            "get_runtime_activation" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(workflow_engine::get_runtime_activation(
                        app.clone(),
                        args.mission_id,
                        args.node_id,
                        args.attempt,
            ...
            "start_agent_run" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(agent_run::start_agent_run(
                    app.clone(),
                    &app.agent_run_state,
                    args.payload,
                )?)
            ...
            "get_agent_run" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                let record = agent_run::get_agent_run(&app.db_state, args.run_id.clone())?
                    .ok_or_else(|| format!("Agent run {} not found.", args.run_id))?;
            ...
            "list_agent_runs" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(agent_run::list_agent_runs(
                    &app.db_state,
                    args.mission_id,
                )?)
                .map_err(|e| e.to_string())?)
            ...
            "get_tasks" => {
                Ok(serde_json::to_value(db::get_tasks(&app.db_state)?)
                    .map_err(|e| e.to_string())?)
            }
            ...
            "add_task" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(db::add_task(
                    args.title,
                    args.description,
                    args.parent_id,
                    args.agent_id,
            ...
            "get_file_locks" => Ok(serde_json::to_value(db::get_file_locks(&app.db_state)?)
                .map_err(|e| e.to_string())?),
            ...
            "get_db_path" => {
                Ok(serde_json::to_value(db::get_db_path(&app.db_state))
                    .map_err(|e| e.to_string())?)
            }
            ...
            "get_session_history" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(db::get_session_history(args.limit, &app.db_state)?)
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "workspace_read_dir" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(workspace::workspace_read_dir(args.path)?)
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "workspace_read_text_file" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(workspace::workspace_read_text_file(args.path)?)
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "workspace_search" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(workspace::workspace_search(args.dir_path, args.query)?)
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "classify_permission_prompt" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(pty::classify_permission_prompt(&args.raw, &args.cli))
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "spawn_pty" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(pty::spawn_pty(
                    app.clone(),
                    &app.pty_state,
                    args.id,
                    args.rows,
            ...
            "spawn_pty_with_command" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(pty::spawn_pty_with_command(
                    app.clone(),
                    &app.pty_state,
                    args.id,
                    args.rows,
            ...
            "get_pty_recent_output" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(pty::get_pty_recent_output(
                    &app.pty_state,
                    args.id,
                    args.max_bytes,
                )?)
            ...
            "is_pty_active" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(
                    serde_json::to_value(pty::is_pty_active(&app.pty_state, args.id))
                        .map_err(|e| e.to_string())?,
                )
            }
            ...
            "list_active_permission_requests" => Ok(serde_json::to_value(
                pty::list_active_permission_requests(&app.pty_state),
            )
            .map_err(|e| e.to_string())?),

            "list_permission_audit_entries" => Ok(serde_json::to_value(
                pty::list_permission_audit_entries(&app.pty_state),
            )
            .map_err(|e| e.to_string())?),
            ...
            "get_mcp_url" => Ok(serde_json::to_value(mcp::get_mcp_url(&app.mcp_state))
                .map_err(|e| e.to_string())?),
            ...
            "mcp_register_runtime_session" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(mcp::mcp_register_runtime_session(
                    &app.mcp_state,
                    args.payload,
                )?)
                .map_err(|e| e.to_string())?)
            ...
            "mcp_notify_agent" => {
                ...
                let args: Args =
                    serde_json::from_value(req.payload.clone()).map_err(|e| e.to_string())?;
                Ok(serde_json::to_value(mcp::mcp_notify_agent(
                    &app.mcp_state,
                    args.session_id,
                    args.kind,
                    args.mission_id,
