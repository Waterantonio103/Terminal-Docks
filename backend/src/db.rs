...
pub fn lock_file(file_path: String, agent_id: String, state: &DbState) -> Result<(), String> {
    let db_lock = state
        .db
        .lock()
        .map_err(|_| "Failed to lock database".to_string())?;
...
pub fn unlock_file(file_path: String, agent_id: String, state: &DbState) -> Result<(), String> {
    let db_lock = state
        .db
        .lock()
        .map_err(|_| "Failed to lock database".to_string())?;
